...
class Functions():
    ...
    def draw_text_on_cv_frame(title, text, position, font, font_size, color, line_size):
        cv2.putText(streams[title, 'img'], text, position, font, font_size, [0, 0, 0], line_size + 5)
        cv2.putText(streams[title, 'img'], text, position, font, font_size, color, line_size)
        #return frame
    ...
    def write_header(tornadoself):
        ...
        # tornadoself.set_header("Content-Type", "video/x-motion-jpeg;boundary=--boundarydonotcross")
...
class StreamControl():
    ...
    def init_thread(name):
        ...
        if name == "color":
            title = "drone3-2.mjpg"
            stream_path = "rtsp://172.19.1.10/user=admin_password=tlJwpbo6_channel=1_stream=0.sdp?real_stream"
    ...
    def init_threads():
        ...
        # streams_django = Stream.objects.all()
        StreamControl.init_thread("termal")
        StreamControl.init_thread("color")
        # for stream in streams_django:
        #     StreamControl.init_thread(stream)
    ...
    def start_threads():
        ...
        flag_ok = True
        for stream in streams_django:
            flag_ok &= (StreamControl.start_thread(stream.title).find("successful") != -1 or StreamControl.start_thread(
                stream.title).find(
                "already") != -1)
...
def handleFrame(title):
    ...
    print("w = " + str(w))
    print("fps = " + str(fps))
    # На случай если камера недоступна
    ...
    while True:
        ...
        if ret:
            ...
            if streams[title, "telemetry_required"] == True:
                Functions.draw_text_on_cv_frame(frame, "Altutide: 10 m.", (10, 23), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                                                [255, 255, 255], 2)
                Functions.draw_text_on_cv_frame(frame, "Speed: 5 m/s", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                                                [255, 255, 255], 2)
...
class APIHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        if action == "start":
            Functions.write_status(self, Functions.start_thread(title))
        elif action == "stop":
            Functions.write_status(self, Functions.stop_thread(title))
        elif action == "restart":
            Functions.write_status(self, Functions.restart_thread(title))
        elif action == "start-all":
            Functions.write_status(self, Functions.start_threads())
        elif action == "stop-all":
            Functions.write_status(self, Functions.stop_threads())
        elif action == "restart-all":
            Functions.write_status(self, Functions.restart_threads())

        elif action == "create":
            json_parsed = json.loads(obj)
            ilog(str(json_parsed))
            new_stream = Stream.create(json_parsed['title'], json_parsed['stream_in'], json_parsed['nn_required'],
                                       json_parsed['width'],
                                       json_parsed['height'],
                                       json_parsed['fps'])
            new_stream.save()
            log(
                "New stream successfully created. title - %s, stream_in - %s, nn_required - %s, width - %s, height - %s, fps - %s" % (
                    json_parsed['title'], json_parsed['stream_in'], json_parsed['nn_required'], json_parsed['width'],
                    json_parsed['height'],
                    json_parsed['fps']),
                "green")
