...
class DroneThread(Thread):
    ...
    async def statistic_thread(self):
        ...
        try:
            ...
            file_logger.info(e)
    async def async_commands_thread(self):
        try:
            ...
            if len(commands) != 0:
                for command in commands:
                    if command.type == 'cancel':
                        ...
                        cmds_cancelled = DroneCommand.objects.filter(status="4", route_uid=route_cancelled.uid,
                                                                     type='waypoint')
                        ...
                        while len(cmds_cancelled) != len(all_cmds):
                            ...
                            if ex == None:
                                exchange_object = ExchangeObject(type="command", uid=command.uid)
                                exchange_object.save()
                            commands = DroneCommand.objects.filter(status__in=["0", "1", "2", "3"], type='waypoint',
                                                                   route_uid=command.route_uid).order_by('outer_id')
                            route_uid = ""
                            for command in commands:
                                command.status = "4"
                                command.save(update_fields=['status'])
            ...
            file_logger.info(e)
            file_logger.info("***************** </ error > ******************")
    async def commands_thread(self):
        ...
        try:
            ...
            if self.drone.connected == True:
                ...
                if len(current_routes) > 0:
                    ...
                    counter = DroneCommand.objects.filter(route_uid=current_route.uid, type='waypoint',
                                                          status__in=['1', '2'])
                    if len(counter) == len(json.loads(current_route.coordinates)):
                        commands = DroneCommand.objects.filter(status="2", is_async=False, type='waypoint',
                                                               route_uid=current_route.uid).order_by(
                            'outer_id')  # Сначала ищем выполняющиеся в текущий момент (если было потеряно соединение)
                        if len(commands) > 0:
                            self.go_to_commands(commands)
                            issetCommands = True
                        commands = DroneCommand.objects.filter(status="1", is_async=False, type='waypoint',
                                                               route_uid=current_route.uid).order_by(
                            'outer_id')  # Идем к следующей точке
                        if len(commands) > 0:
                            self.go_to_commands(commands)
                            issetCommands = True
                        if issetCommands == True:
                            ...
                            if self.cancel == True:
                                ...
                                ex = ExchangeObject.objects.filter(type="route", uid=current_route.uid).last()
                                if ex == None:
            ...
            file_logger.info(e)
            file_logger.info("***************** </ error > ******************")
    async def exchange_thread(self):
        data_recieved = {}
        # ***************************************** RECIEVED *********************************************
    ...
    async def Commands(self):
        try:
            ...
            file_logger.info(str(e))
            return
    async def AsyncCommands(self):
        try:
            while self.drone.connected:
                #await asyncio.sleep(0.001)
                futures = [self.async_commands_thread()]
                for future in asyncio.as_completed(futures):
                    result = await future
            file_logger.info("statistic or comamnds thread is not alive...")
            self.vehicle.close()
    ...
    async def connectMain(self):
        ...
        while self.drone.connected == False:
            try:
                ...
                while self.drone.connected:
                    await asyncio.sleep(0.001)
                    #threads_is_alive = await self.statistic_thread.is_alive() and self.commands_thread.is_alive() and self.exchange_thread.is_alive()
                    if self.vehicle.last_heartbeat > 15:
                        ...
                        self.drone.connected = True
                        futures = [self.statistic_thread(), self.commands_thread(), self.async_commands_thread(), self.exchange_thread()]
                        for future in asyncio.as_completed(futures):
                            result = await future
                        await asyncio.sleep(1)
                file_logger.info("statistic or comamnds thread is not alive...")
    ...
    def run(self):
        ...
        if self.name == "main_thread":
            asyncio.run(self.connectMain())
            asyncio.run(self.Statistic())
            asyncio.run(self.Commands())
            asyncio.run(self.AsyncCommands())
